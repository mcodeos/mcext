...
fn completion_in_pins_list_returns_items() {
    // Verify P4.1: completion in helper.mc's `pins = [` returns non-empty items
    let state = WorkspaceState::new();
    let helper_url = fixture_uri("helper.mc");
    load_into_state(&state, &helper_url, "helper.mc");
    ...
    assert!(result.is_some(), "completion should return Some");
...
fn completion_returns_keywords_at_general_position() {
    ...
    assert!(
        items.iter().any(|i| i.label == "component"),
        "General context should include component keyword"
    );
}

#[test]
fn goto_definition_does_not_panic_at_instance_ref() {
    // Verify P1: goto_definition doesn't panic at main.mc's helper_chip hc1 reference
    let state = WorkspaceState::new();
    let main_url = fixture_uri("main.mc");
    load_into_state(&state, &main_url, "main.mc");

    // hc1 is at approximately line 5 in main.mc
...
fn semantic_tokens_compute_runs_on_real_parse() {
    ...
    // Passes as long as it doesn't panic; empty array is acceptable (no error on parse failure)
    let _ = tokens;
}
