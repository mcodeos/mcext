...
fn resolve_use_jump(
    ...
    info!("goto_def: use_jump parsed: prefix={:?}, use_path={:?}", prefix, use_path);
    ...
    let range = Range::new(
        ...
        Position::new(line_idx as u32, (range_start_in_line + path_only.len()) as u32),
    );
    
    info!("goto_def: use_jump SUCCESS: returning {:?}", target_url);
    Some(GotoDefinitionResponse::Scalar(Location::new(target_url, range)))
}
